...
class AddNewItem extends React.Component {
	...
	_getTotalBudget() {
		return "$" + WalletStore.getTotalBudget()
	}
	...
	_addNewItem(event) {
		...
		this.state.item.description = this.state.item.description || '_';
		this.state.item.amount = this.state.item.amount || '0';
		WalletActions.addNewItem(this.state.item);
		this.setState({ item: this._getFreshItem(), total: this._getTotalBudget()});
	}
		...
		return (
			<div>
				...
				<form className="form-inline add-item" onSubmit={this._addNewItem.bind(this)}>
					<input type="text" className="form-control description" name="description" 
						value={this.state.item.description} placeholder="Description" onChange={this._updateState.bind(this)} />
					<div className="input-group amount">
						<div className="input-group-addon">$</div>
						<input type="text" className="form-control" name="amount" 
							value={this.state.item.amount} placeholder="Amount" onChange={this._updateState.bind(this)} />
					</div>
